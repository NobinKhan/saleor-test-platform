...
from app.routes.tests import create_run, validate_target
...
@pytest.mark.asyncio
async def test_validate_clones_password_from_source_run():
    user_id = uuid.uuid4()
    source_id = uuid.uuid4()
    user = User(id=user_id, email="test@example.com", name="Test", password_hash="x")
    ...
    with patch("app.routes.tests.authenticate_saleor", new_callable=AsyncMock) as mock_auth:
        mock_auth.return_value = ("new-jwt", None)
        with patch(
            "app.services.fixture_resolver.validate_preflight",
            new_callable=AsyncMock,
        ) as mock_preflight:
            mock_preflight.return_value = {"issues": [], "version_gate_pass": True}
            result = await validate_target(data, db=db, user=user)
